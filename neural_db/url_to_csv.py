...
def create_train_df(elements, id_col):
    ...
    df["text"] = df["text"].apply(lambda text: text.encode("utf-8", "replace").decode("utf-8"))
    return df
...
def url_to_df(base_url: str, id_col: str, scrape_depth: int):
    all_elements = []

    urls = get_all_urls(base_url, max_crawl_depth=scrape_depth)

    for url, response in urls:
        elem, success = process_url(url, response)

        if success:
            all_elements.extend(
                [
                    (*el, i)
                    for el, i in zip(elem, range(len(elem)))
                ]
            )
    ...
    all_samples_df = create_train_df(all_elements, id_col=id_col)
...
url_to_csv("https://profiles.rice.edu/faculty/", "DOC_ID", "faculties.csv", 4)